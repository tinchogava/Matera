/**
 * This class is generated by jOOQ
 */
package matera.jooq.routines;


import javax.annotation.Generated;

import matera.jooq.Matera;

import org.jooq.Parameter;
import org.jooq.impl.AbstractRoutine;


/**
 * This class is generated by jOOQ.
 */
@Generated(
	value = {
		"http://www.jooq.org",
		"jOOQ version:3.7.2"
	},
	comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class Insertproductofact extends AbstractRoutine<java.lang.Void> {

	private static final long serialVersionUID = 1090549694;

	/**
	 * The parameter <code>matera.insertProductoFact.id_productoFact</code>.
	 */
	public static final Parameter<Integer> ID_PRODUCTOFACT = createParameter("id_productoFact", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.insertProductoFact.codigo</code>.
	 */
	public static final Parameter<String> CODIGO = createParameter("codigo", org.jooq.impl.SQLDataType.VARCHAR.length(10), false);

	/**
	 * The parameter <code>matera.insertProductoFact.nombre</code>.
	 */
	public static final Parameter<String> NOMBRE = createParameter("nombre", org.jooq.impl.SQLDataType.VARCHAR.length(45), false);

	/**
	 * The parameter <code>matera.insertProductoFact.id_especialidad</code>.
	 */
	public static final Parameter<Integer> ID_ESPECIALIDAD = createParameter("id_especialidad", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.insertProductoFact.id_subespecialidad</code>.
	 */
	public static final Parameter<Integer> ID_SUBESPECIALIDAD = createParameter("id_subespecialidad", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.insertProductoFact.id_forecastGrupo</code>.
	 */
	public static final Parameter<Integer> ID_FORECASTGRUPO = createParameter("id_forecastGrupo", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.insertProductoFact.habilita</code>.
	 */
	public static final Parameter<String> HABILITA = createParameter("habilita", org.jooq.impl.SQLDataType.VARCHAR.length(1), false);

	/**
	 * Create a new routine call instance
	 */
	public Insertproductofact() {
		super("insertProductoFact", Matera.MATERA);

		addInParameter(ID_PRODUCTOFACT);
		addInParameter(CODIGO);
		addInParameter(NOMBRE);
		addInParameter(ID_ESPECIALIDAD);
		addInParameter(ID_SUBESPECIALIDAD);
		addInParameter(ID_FORECASTGRUPO);
		addInParameter(HABILITA);
	}

	/**
	 * Set the <code>id_productoFact</code> parameter IN value to the routine
	 */
	public void setIdProductofact(Integer value) {
		setValue(ID_PRODUCTOFACT, value);
	}

	/**
	 * Set the <code>codigo</code> parameter IN value to the routine
	 */
	public void setCodigo(String value) {
		setValue(CODIGO, value);
	}

	/**
	 * Set the <code>nombre</code> parameter IN value to the routine
	 */
	public void setNombre(String value) {
		setValue(NOMBRE, value);
	}

	/**
	 * Set the <code>id_especialidad</code> parameter IN value to the routine
	 */
	public void setIdEspecialidad(Integer value) {
		setValue(ID_ESPECIALIDAD, value);
	}

	/**
	 * Set the <code>id_subespecialidad</code> parameter IN value to the routine
	 */
	public void setIdSubespecialidad(Integer value) {
		setValue(ID_SUBESPECIALIDAD, value);
	}

	/**
	 * Set the <code>id_forecastGrupo</code> parameter IN value to the routine
	 */
	public void setIdForecastgrupo(Integer value) {
		setValue(ID_FORECASTGRUPO, value);
	}

	/**
	 * Set the <code>habilita</code> parameter IN value to the routine
	 */
	public void setHabilita(String value) {
		setValue(HABILITA, value);
	}
}
