/**
 * This class is generated by jOOQ
 */
package matera.jooq.routines;


import java.sql.Date;

import javax.annotation.Generated;

import matera.jooq.Matera;

import org.jooq.Parameter;
import org.jooq.impl.AbstractRoutine;


/**
 * This class is generated by jOOQ.
 */
@Generated(
	value = {
		"http://www.jooq.org",
		"jOOQ version:3.7.2"
	},
	comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class Consultamovimientosstock extends AbstractRoutine<java.lang.Void> {

	private static final long serialVersionUID = 455380004;

	/**
	 * The parameter <code>matera.consultaMovimientosStock.id_empresa</code>.
	 */
	public static final Parameter<Integer> ID_EMPRESA = createParameter("id_empresa", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.consultaMovimientosStock.ids</code>.
	 */
	public static final Parameter<String> IDS = createParameter("ids", org.jooq.impl.SQLDataType.CLOB, false);

	/**
	 * The parameter <code>matera.consultaMovimientosStock.desde</code>.
	 */
	public static final Parameter<Date> DESDE = createParameter("desde", org.jooq.impl.SQLDataType.DATE, false);

	/**
	 * The parameter <code>matera.consultaMovimientosStock.hasta</code>.
	 */
	public static final Parameter<Date> HASTA = createParameter("hasta", org.jooq.impl.SQLDataType.DATE, false);

	/**
	 * The parameter <code>matera.consultaMovimientosStock.lote</code>.
	 */
	public static final Parameter<String> LOTE = createParameter("lote", org.jooq.impl.SQLDataType.VARCHAR.length(45), false);

	/**
	 * The parameter <code>matera.consultaMovimientosStock.serie</code>.
	 */
	public static final Parameter<String> SERIE = createParameter("serie", org.jooq.impl.SQLDataType.VARCHAR.length(45), false);

	/**
	 * The parameter <code>matera.consultaMovimientosStock.id_zona</code>.
	 */
	public static final Parameter<Integer> ID_ZONA = createParameter("id_zona", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.consultaMovimientosStock.dc</code>.
	 */
	public static final Parameter<String> DC = createParameter("dc", org.jooq.impl.SQLDataType.VARCHAR.length(1), false);

	/**
	 * Create a new routine call instance
	 */
	public Consultamovimientosstock() {
		super("consultaMovimientosStock", Matera.MATERA);

		addInParameter(ID_EMPRESA);
		addInParameter(IDS);
		addInParameter(DESDE);
		addInParameter(HASTA);
		addInParameter(LOTE);
		addInParameter(SERIE);
		addInParameter(ID_ZONA);
		addInParameter(DC);
	}

	/**
	 * Set the <code>id_empresa</code> parameter IN value to the routine
	 */
	public void setIdEmpresa(Integer value) {
		setValue(ID_EMPRESA, value);
	}

	/**
	 * Set the <code>ids</code> parameter IN value to the routine
	 */
	public void setIds(String value) {
		setValue(IDS, value);
	}

	/**
	 * Set the <code>desde</code> parameter IN value to the routine
	 */
	public void setDesde(Date value) {
		setValue(DESDE, value);
	}

	/**
	 * Set the <code>hasta</code> parameter IN value to the routine
	 */
	public void setHasta(Date value) {
		setValue(HASTA, value);
	}

	/**
	 * Set the <code>lote</code> parameter IN value to the routine
	 */
	public void setLote(String value) {
		setValue(LOTE, value);
	}

	/**
	 * Set the <code>serie</code> parameter IN value to the routine
	 */
	public void setSerie(String value) {
		setValue(SERIE, value);
	}

	/**
	 * Set the <code>id_zona</code> parameter IN value to the routine
	 */
	public void setIdZona(Integer value) {
		setValue(ID_ZONA, value);
	}

	/**
	 * Set the <code>dc</code> parameter IN value to the routine
	 */
	public void setDc(String value) {
		setValue(DC, value);
	}
}
