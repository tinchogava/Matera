/**
 * This class is generated by jOOQ
 */
package matera.jooq.routines;


import java.math.BigDecimal;
import java.sql.Date;

import javax.annotation.Generated;

import matera.jooq.Matera;

import org.jooq.Parameter;
import org.jooq.impl.AbstractRoutine;


/**
 * This class is generated by jOOQ.
 */
@Generated(
	value = {
		"http://www.jooq.org",
		"jOOQ version:3.7.2"
	},
	comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class Guardaconfirmaturno extends AbstractRoutine<java.lang.Void> {

	private static final long serialVersionUID = 1868734563;

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.id_presupuesto</code>.
	 */
	public static final Parameter<Integer> ID_PRESUPUESTO = createParameter("id_presupuesto", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.id_tecnico</code>.
	 */
	public static final Parameter<Integer> ID_TECNICO = createParameter("id_tecnico", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.id_prestacion</code>.
	 */
	public static final Parameter<Integer> ID_PRESTACION = createParameter("id_prestacion", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.importePrestacion</code>.
	 */
	public static final Parameter<BigDecimal> IMPORTEPRESTACION = createParameter("importePrestacion", org.jooq.impl.SQLDataType.DECIMAL.precision(9, 2), false);

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.observaciones</code>.
	 */
	public static final Parameter<String> OBSERVACIONES = createParameter("observaciones", org.jooq.impl.SQLDataType.CLOB, false);

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.id_entidad</code>.
	 */
	public static final Parameter<Integer> ID_ENTIDAD = createParameter("id_entidad", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.id_lugarCirugia</code>.
	 */
	public static final Parameter<Integer> ID_LUGARCIRUGIA = createParameter("id_lugarCirugia", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.id_profesional1</code>.
	 */
	public static final Parameter<Integer> ID_PROFESIONAL1 = createParameter("id_profesional1", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.fechaCirugia</code>.
	 */
	public static final Parameter<Date> FECHACIRUGIA = createParameter("fechaCirugia", org.jooq.impl.SQLDataType.DATE, false);

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.fechaAgenda</code>.
	 */
	public static final Parameter<Date> FECHAAGENDA = createParameter("fechaAgenda", org.jooq.impl.SQLDataType.DATE, false);

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.paciente</code>.
	 */
	public static final Parameter<String> PACIENTE = createParameter("paciente", org.jooq.impl.SQLDataType.VARCHAR.length(45), false);

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.usuario</code>.
	 */
	public static final Parameter<String> USUARIO = createParameter("usuario", org.jooq.impl.SQLDataType.VARCHAR.length(45), false);

	/**
	 * The parameter <code>matera.guardaConfirmaTurno.equipo</code>.
	 */
	public static final Parameter<String> EQUIPO = createParameter("equipo", org.jooq.impl.SQLDataType.VARCHAR.length(255), false);

	/**
	 * Create a new routine call instance
	 */
	public Guardaconfirmaturno() {
		super("guardaConfirmaTurno", Matera.MATERA);

		addInParameter(ID_PRESUPUESTO);
		addInParameter(ID_TECNICO);
		addInParameter(ID_PRESTACION);
		addInParameter(IMPORTEPRESTACION);
		addInParameter(OBSERVACIONES);
		addInParameter(ID_ENTIDAD);
		addInParameter(ID_LUGARCIRUGIA);
		addInParameter(ID_PROFESIONAL1);
		addInParameter(FECHACIRUGIA);
		addInParameter(FECHAAGENDA);
		addInParameter(PACIENTE);
		addInParameter(USUARIO);
		addInParameter(EQUIPO);
	}

	/**
	 * Set the <code>id_presupuesto</code> parameter IN value to the routine
	 */
	public void setIdPresupuesto(Integer value) {
		setValue(ID_PRESUPUESTO, value);
	}

	/**
	 * Set the <code>id_tecnico</code> parameter IN value to the routine
	 */
	public void setIdTecnico(Integer value) {
		setValue(ID_TECNICO, value);
	}

	/**
	 * Set the <code>id_prestacion</code> parameter IN value to the routine
	 */
	public void setIdPrestacion(Integer value) {
		setValue(ID_PRESTACION, value);
	}

	/**
	 * Set the <code>importePrestacion</code> parameter IN value to the routine
	 */
	public void setImporteprestacion(BigDecimal value) {
		setValue(IMPORTEPRESTACION, value);
	}

	/**
	 * Set the <code>observaciones</code> parameter IN value to the routine
	 */
	public void setObservaciones(String value) {
		setValue(OBSERVACIONES, value);
	}

	/**
	 * Set the <code>id_entidad</code> parameter IN value to the routine
	 */
	public void setIdEntidad(Integer value) {
		setValue(ID_ENTIDAD, value);
	}

	/**
	 * Set the <code>id_lugarCirugia</code> parameter IN value to the routine
	 */
	public void setIdLugarcirugia(Integer value) {
		setValue(ID_LUGARCIRUGIA, value);
	}

	/**
	 * Set the <code>id_profesional1</code> parameter IN value to the routine
	 */
	public void setIdProfesional1(Integer value) {
		setValue(ID_PROFESIONAL1, value);
	}

	/**
	 * Set the <code>fechaCirugia</code> parameter IN value to the routine
	 */
	public void setFechacirugia(Date value) {
		setValue(FECHACIRUGIA, value);
	}

	/**
	 * Set the <code>fechaAgenda</code> parameter IN value to the routine
	 */
	public void setFechaagenda(Date value) {
		setValue(FECHAAGENDA, value);
	}

	/**
	 * Set the <code>paciente</code> parameter IN value to the routine
	 */
	public void setPaciente(String value) {
		setValue(PACIENTE, value);
	}

	/**
	 * Set the <code>usuario</code> parameter IN value to the routine
	 */
	public void setUsuario(String value) {
		setValue(USUARIO, value);
	}

	/**
	 * Set the <code>equipo</code> parameter IN value to the routine
	 */
	public void setEquipo(String value) {
		setValue(EQUIPO, value);
	}
}
