/**
 * This class is generated by jOOQ
 */
package matera.jooq.routines;


import javax.annotation.Generated;

import matera.jooq.Matera;

import org.jooq.Parameter;
import org.jooq.impl.AbstractRoutine;


/**
 * This class is generated by jOOQ.
 */
@Generated(
	value = {
		"http://www.jooq.org",
		"jOOQ version:3.7.2"
	},
	comments = "This class is generated by jOOQ"
)
@SuppressWarnings({ "all", "unchecked", "rawtypes" })
public class Detalleforecast extends AbstractRoutine<java.lang.Void> {

	private static final long serialVersionUID = 150516842;

	/**
	 * The parameter <code>matera.detalleForecast.id_vendedor</code>.
	 */
	public static final Parameter<Integer> ID_VENDEDOR = createParameter("id_vendedor", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.detalleForecast.id_entidad</code>.
	 */
	public static final Parameter<Integer> ID_ENTIDAD = createParameter("id_entidad", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.detalleForecast.id_productoFact</code>.
	 */
	public static final Parameter<Integer> ID_PRODUCTOFACT = createParameter("id_productoFact", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.detalleForecast.anio</code>.
	 */
	public static final Parameter<Integer> ANIO = createParameter("anio", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.detalleForecast.mes</code>.
	 */
	public static final Parameter<Integer> MES = createParameter("mes", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.detalleForecast.tipo</code>.
	 */
	public static final Parameter<Integer> TIPO = createParameter("tipo", org.jooq.impl.SQLDataType.INTEGER, false);

	/**
	 * The parameter <code>matera.detalleForecast.vip</code>.
	 */
	public static final Parameter<Byte> VIP = createParameter("vip", org.jooq.impl.SQLDataType.TINYINT, false);

	/**
	 * Create a new routine call instance
	 */
	public Detalleforecast() {
		super("detalleForecast", Matera.MATERA);

		addInParameter(ID_VENDEDOR);
		addInParameter(ID_ENTIDAD);
		addInParameter(ID_PRODUCTOFACT);
		addInParameter(ANIO);
		addInParameter(MES);
		addInParameter(TIPO);
		addInParameter(VIP);
	}

	/**
	 * Set the <code>id_vendedor</code> parameter IN value to the routine
	 */
	public void setIdVendedor(Integer value) {
		setValue(ID_VENDEDOR, value);
	}

	/**
	 * Set the <code>id_entidad</code> parameter IN value to the routine
	 */
	public void setIdEntidad(Integer value) {
		setValue(ID_ENTIDAD, value);
	}

	/**
	 * Set the <code>id_productoFact</code> parameter IN value to the routine
	 */
	public void setIdProductofact(Integer value) {
		setValue(ID_PRODUCTOFACT, value);
	}

	/**
	 * Set the <code>anio</code> parameter IN value to the routine
	 */
	public void setAnio(Integer value) {
		setValue(ANIO, value);
	}

	/**
	 * Set the <code>mes</code> parameter IN value to the routine
	 */
	public void setMes(Integer value) {
		setValue(MES, value);
	}

	/**
	 * Set the <code>tipo</code> parameter IN value to the routine
	 */
	public void setTipo(Integer value) {
		setValue(TIPO, value);
	}

	/**
	 * Set the <code>vip</code> parameter IN value to the routine
	 */
	public void setVip(Byte value) {
		setValue(VIP, value);
	}
}
