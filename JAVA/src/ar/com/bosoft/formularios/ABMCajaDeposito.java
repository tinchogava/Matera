...
public class ABMCajaDeposito extends javax.swing.JInternalFrame {
    Conexion conexion;
    ...
    ArrayList arrayOpcion, arrayZona, arrayCaja, arrayCajaDeposito;
    
    String empresa;
    int id_empresa, id_cajaDeposito;
     ...
     * @param empresa
     */
    public ABMCajaDeposito(Conexion conexion, int id_empresa, String empresa) {
    ...
    private void llenaComboCaja(){
        ...
        if (arrayCaja.isEmpty()){
            JOptionPane.showMessageDialog(this,"No hay cajas habilitadas", "Atención",JOptionPane.INFORMATION_MESSAGE);
        }else{
    ...
    private void limpia(){
        ...
        this.jComboCaja.setSelectedIndex(arrayCaja.isEmpty() ? -1 : 0);
    ...
    private void consulta(){
        ...
        arrayCajaDeposito = cajaDepositoCRUD.consulta(false);
        Iterator i = arrayCajaDeposito.iterator();
        while (i.hasNext()){
            CajaDepositoData tmp = (CajaDepositoData) i.next();
            
            String codigo = tmp.getCodigo();            
            String caja = tmp.getCaja();
            String zona = tmp.getZona();            
            String habilitado = tmp.getHabilita().equals("S") ? "SI" : "NO";
            
            Object [] nuevo = {codigo,caja,zona,habilitado};
            modelo.addRow(nuevo);
        }
    }
    ...
    private boolean validaObligatorios(){
        return this.jComboZona.getSelectedIndex() >= 0 && 
                this.jComboCaja.getSelectedIndex() >= 0 && 
    ...
    private boolean existeCaja(){
        ...
        for (int i = 0; i < reg; i++){
            Object compara = modelo.getValueAt(i, 0);
            if (compara.equals(codigo)){
                compara = modelo.getValueAt(i, 2);
                if (compara.equals(zona)) {
                    return true;
                }
            }
        }
    ...
    private void initComponents() {
        ...
        if (jTabla.getColumnModel().getColumnCount() > 0) {
            jTabla.getColumnModel().getColumn(0).setMinWidth(70);
            jTabla.getColumnModel().getColumn(0).setPreferredWidth(70);
            jTabla.getColumnModel().getColumn(0).setMaxWidth(70);
                            ...
                            .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                                .addGroup(layout.createSequentialGroup()
                                    ...
                                    .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                                        .addComponent(jComboZona, 0, 230, Short.MAX_VALUE)
...
//            if (!existeCaja()){
                CajaDepositoData nuevo = new CajaDepositoData();

                nuevo.setId_cajaDeposito(this.id_cajaDeposito);

                ZonaData zonaVO = (ZonaData) arrayZona.get(this.jComboZona.getSelectedIndex());
                nuevo.setId_zona(zonaVO.getId_zona());

                CajaData cajaVO = (CajaData) arrayCaja.get(this.jComboCaja.getSelectedIndex());
                nuevo.setId_caja(cajaVO.getId_caja());

                nuevo.setCodigo(this.jTxtCodigo.getText());
                nuevo.setHabilita(this.jComboHabilita.getSelectedItem().toString().substring(0, 1));

                cajaDepositoCRUD.insert(nuevo);
                
                this.limpia();
                this.consulta();
                this.jTxtCodigo.requestFocus();
//            }else{
//                JOptionPane.showMessageDialog(this, "Ya existe una caja con ese código", "Atención", JOptionPane.INFORMATION_MESSAGE);
//            }                
        }else{
    ...
    private void jTablaMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jTablaMouseClicked
        ...
        CajaDepositoData tmp = (CajaDepositoData) arrayCajaDeposito.get(indice);
        
        this.id_cajaDeposito = tmp.getId_cajaDeposito();
        
        this.jComboZona.setSelectedIndex(-1);
        this.jComboZona.setSelectedItem(tmp.getZona());
        
        this.jComboCaja.setSelectedIndex(-1);
        this.jComboCaja.setSelectedItem(tmp.getCaja());
