...
//
    ...
    public List<T> getRowsAsList(List<Integer> rows) {
        ArrayList<T> rowData = new ArrayList<>(rows.size());
        rows.forEach(r->rowData.add(getRow(r)));
        return rowData;
    }    

    public List<T> getRowsAsList() {
        ArrayList<T> rowData = new ArrayList<T>(getRowCount());
        for (int i = 0; i < getRowCount(); i++) {
            rowData.add(getRow(i));
        }
        return rowData;
    }
    ...
    public void getPropertiesFromDefaultModel(TableModel model) {
        List<String> columns = new ArrayList<>();
        Boolean[] editable = new Boolean[model.getColumnCount()];
        for (int i = 0; i < model.getColumnCount(); i++) {
            columns.add(model.getColumnName(i));
            editable[i] = model.isCellEditable(0, i);
        }
        setDataAndColumnNames(new ArrayList<>(), columns);
        this.isColumnEditable = editable;
        fireTableStructureChanged();
    }
}
